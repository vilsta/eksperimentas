...
const participantSessionColumns = new Set(
  db.prepare('PRAGMA table_info(participant_sessions)').all().map(column => column.name)
);
const participantSessionMigrations = [
  ['age_group', "TEXT DEFAULT ''"],
  ['gender', "TEXT DEFAULT ''"],
  ['education_level', "TEXT DEFAULT ''"],
  ['professional_field', "TEXT DEFAULT ''"],
...
for (const [name, definition] of participantSessionMigrations) {
  if (!participantSessionColumns.has(name)) {
    db.exec(`ALTER TABLE participant_sessions ADD COLUMN ${name} ${definition}`);
  }
}
