...
async function api(req,res,pathname){
  ...
  if(pathname==='/api/admin/export.csv'&&method==='GET'){
    requireAdmin(req);const rows=db.prepare('SELECT s.participant_label participant,s.id session_id,s.device_type,s.experience_level,q.id question_id,q.task_type,q.answer_format,a.answer_json,a.is_correct,a.elapsed_seconds,a.clarity,a.appeal,a.difficulty,a.submitted_at FROM answers a JOIN participant_sessions s ON s.id=a.session_id JOIN questions q ON q.id=a.question_id ORDER BY s.started_at,q.id').all();const headers=['participant','session_id','device_type','experience_level','question_id','task_type','answer_format','answer','is_correct','elapsed_seconds','clarity','appeal','difficulty','submitted_at'];const csvEsc=v=>'"'+String(v??'').replaceAll('"','""')+'"';const csv='\ufeff'+[headers.map(csvEsc).join(','),...rows.map(r=>headers.map(h=>csvEsc(h==='answer'?parseJSON(r.answer_json,[]).join(' | '):r[h])).join(','))].join('\n');return sendText(res,200,csv,'text/csv; charset=utf-8',{'Content-Disposition':'attachment; filename="eksperimento_rezultatai.csv"'});
  }

  if(pathname==='/api/public/start'&&method==='POST'){
    const b=await bodyJson(req),exp=db.prepare("SELECT * FROM experiments WHERE status='Aktyvus' ORDER BY id DESC LIMIT 1").get();if(!exp)throw httpError(404,'Šiuo metu nėra aktyvaus eksperimento.');let ids=db.prepare("SELECT q.id FROM experiment_questions eq JOIN questions q ON q.id=eq.question_id WHERE eq.experiment_id=? AND q.status='Aktyvus' ORDER BY eq.position").all(exp.id).map(x=>x.id);if(exp.order_mode==='Atsitiktinė')ids=shuffle(ids);const id=crypto.randomUUID(),n=db.prepare('SELECT COUNT(*) c FROM participant_sessions').get().c+1,label=`Dalyvis #${n}`,ua=req.headers['user-agent']||'',device=text(b.device)||(/Mobi|Android|iPhone|iPad/i.test(ua)?'Mobilusis':'Kompiuteris'),experience=Number(b.experience)||null;db.prepare('INSERT INTO participant_sessions(id,experiment_id,participant_label,device_type,experience_level,status,question_order_json,started_at,user_agent) VALUES(?,?,?,?,?,?,?,?,?)').run(id,exp.id,label,device,experience,'Vykdoma',JSON.stringify(ids),nowIso(),ua);const questions=ids.map(qid=>publicQuestion(questionToObject(db.prepare('SELECT * FROM questions WHERE id=?').get(qid),true)));return json(res,201,{sessionId:id,participantLabel:label,experiment:{id:exp.id,title:exp.title,description:exp.description},questions});
  }
